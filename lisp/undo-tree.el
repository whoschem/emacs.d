...
may crash if undo history exceeds Emacs' available memory."
  :group 'undo-tree
...
;; Store current horizontal spacing needed for drawing undo-tree.
...
;; Hold node that was current when visualizer was invoked.
...
;; Hold currently selected node in visualizer selection mode.
...
(cl-defstruct
    (undo-tree-node
     ...
     (:constructor undo-tree-copy-node-save-data
                   (node
                    &aux
                    (undo (changeset (undo-tree-node-undo node)))
                    (redo (changeset (undo-tree-node-redo node)))
