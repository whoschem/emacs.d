...
When non-nil, for a key sequence to trigger the which-key popup it
...
(defvar which-key-keymap-history nil
  "History of keymap selections in functions like `which-key-show-keymap'.")
...
(defvar which-key--last-try-2-loc nil
  "Internal: Last location of side-window when two locations used.")
(defvar which-key--automatic-display nil
  "Internal: Non-nil if popup was triggered with automatic update.")
...
starter kit for example."
  ...
  (setq which-key-replacement-alist
        (delete '("left" . "←") which-key-replacement-alist))
  (setq which-key-replacement-alist
        (delete '("right" . "→") which-key-replacement-alist)))
...
of the intended popup.  SELECTED-WINDOW-WIDTH is the width of currently
active window, not the which-key buffer window."
  (cl-case which-key-popup-type
...
Sort single characters alphabetically with lowercase coming
before upper."
...
Use `string-lessp' after applying lowercase."
     ...
     (lambda (ev def)
       (let* ((key (append prefix (list ev)))
         ...
         (cond ((or (string-match-p
                     which-key--ignore-keys-regexp key-desc)
                    (eq ev 'menu-bar)))
               ((and (keymapp def)
                     (or all
...
(defun which-key--show-page (&optional n)
  "Show current page.
N changes the current page to the Nth page relative to the current one."
