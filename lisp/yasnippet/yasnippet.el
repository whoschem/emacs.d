;;; yasnippet.el --- Yet another snippet for Emacs.  -*- lexical-binding: t -*-
...
(defcustom yas-prompt-functions '(yas-dropdown-prompt
                                  ...
                                  yas-no-prompt)
  "Function to prompt for keys, templates, etc interactively.
...
(defcustom yas-indent-line 'auto
  "Control indenting applied to a recent snippet expansion.
...
(defconst yas-not-string-or-comment-condition
  '(if (let ((ppss (syntax-ppss)))
     ...
     t)
  "Disable snippet expansion in strings and comments.
...
(defvar yas--snippet-id-seed 0
  "Contain the next id for a snippet.")
...
\\{yas-minor-mode-map}"
  ...
  (cond ((and yas-minor-mode (featurep 'yasnippet))
         ...
         (add-hook 'auto-fill-mode-hook #'yas--auto-fill-wrapper))
        (t
...
(defun yas-activate-extra-mode (mode)
  "Activate the snippets for the given `mode' in the buffer.
...
(defun yas-deactivate-extra-mode (mode)
  "Deactivate the snippets for the given `mode' in the buffer."
  (interactive
   (list (intern
          (completing-read
...
;;;; structs for template management
...
(defun yas--remove-misc-free-from-undo (old-undo-list)
  "Try to work around Emacs Bug#30931.
...
If no snippet expansion is possible, nothing would happen.
...
(put 'yas-auto-next 'lisp-indent-function 0)
...
(defun yas-new-snippet (&optional no-template)
  "Pop a new buffer for writing a snippet.
...
(defvar yas--current-template nil
  "Hold the current template being expanded into a snippet.")
...
(defvar yas--active-field-overlay nil
  "Overlay the currently active field.")
...
(defun yas-skip-and-clear-field (&optional field)
  "Clear unmodified FIELD if at field start, skips to next tab."
  (interactive)
  (yas--skip-and-clear (or field (yas-current-field)))
  (yas-next-field 1))

(defun yas-clear-field (&optional field)
  "Clear unmodified FIELD if at field start."
  (interactive)
  (yas--skip-and-clear (or field (yas-current-field))))

(defun yas--skip-and-clear (field &optional from)
  "Delete the region of FIELD and sets it's modified state to t.
...
(defun yas--on-field-overlay-modification (overlay after? beg end &optional length)
  "Clear the field and updates mirrors, conditionally.
...
(defun yas--take-care-of-redo (snippet)
  "Commit SNIPPET, which in turn pushes an undo action for reviving it.
...
(defun yas--snippet-revive (beg end snippet)
  "Revive SNIPPET and creates a control overlay from BEG to END.
...
(defun yas--snapshot-location (position &optional beg end)
  "Return info for restoring POSITIONS's location after indent.
...
(defun yas--restore-overlay-location (ov-locations)
  "Restore marker based on info from `yas--snapshot-overlay-location'.
Buffer must be narrowed to BEG..END used to create the snapshot info."
...
(defun yas--restore-overlay-line-location (base-pos ov-locations)
  "Restore marker based on info from `yas--snapshot-overlay-line-location'."
...
(defun yas--post-command-handler ()
  "Handle various yasnippet conditions after each command."
...
(defun yas--expand-from-trigger-key-doc (context)
  ...
  (let* ((fallback-description "do nothing."))
