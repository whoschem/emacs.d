;;; tomorrow-themes.el --- A version of Chris Kempson's "tomorrow" themes  -*- lexical-binding: t -*-
...
;;
;;     M-x tomorrow-themes-day
;;     M-x tomorrow-themes-night
;;     M-x tomorrow-themes-blue
;;     M-x tomorrow-themes-bright
;;     M-x tomorrow-themes-eighties
...
executed."
  (tomorrow--interpolate background highlight 7 3))

(defconst tomorrow-themes-colors
...
(defmacro tomorrow-themes--with-colors (mode &rest body)
...
`MODE' should be set to either 'day, 'night, 'eighties, 'blue or 'bright."
  `(let* ((colors (or (cdr (assoc ,mode tomorrow-themes-colors))
...
(defmacro tomorrow-themes--face-specs ()
   ...
   (mapcar
    ...
    `(;; Standard font lock faces
      ...
      (tab-line-tab (:background ,background
                                 ...
                                 :inverse-video nil
                                 :box (:line-width 1 :color ,contrast-bg)))
      (tab-line-tab-current (:inherit tab-line-tab :foreground ,purple))
      ...
      ;; nerdtree
      (nerdtree-banner-face (:foreground ,blue :weight bold))
      (nerdtree-button-face (:underline t))
      (nerdtree-dir-link-face (:foreground ,orange))
      (nerdtree-expand-btn-face (:foreground ,comment))
      (nerdtree-file-link-face (:foreground ,foreground))
      (nerdtree-header-face (:foreground ,foreground :background ,highlight))
      (nerdtree-root-dir-face (:foreground ,blue :weight bold))
      (nerdtree-vc-added-face (:foreground ,green))
      (nerdtree-vc-conflict-face (:foreground ,red))
      (nerdtree-vc-default-face (:foreground ,foreground))
      (nerdtree-vc-edited-face (:foreground ,purple))
      (nerdtree-vc-ignored-face (:foreground ,contrast-bg))
      (nerdtree-vc-missing-face (:foreground ,red))
      (nerdtree-vc-needs-merge-face (:foreground ,red))
      (nerdtree-vc-unlocked-changes-face (:foreground ,blue :slant italic))
      (nerdtree-vc-user-face (:foreground ,red :slant italic))
...
(defmacro tomorrow-themes--frame-parameter-specs ()
  ...
  (if (fboundp 'load-theme)
    ...
    (progn
      ...
      (tomorrow-themes--with-colors
       mode
       (color-theme-install
        `(,(intern (concat "tomorrow-themes-" (symbol-name mode)))
          ,@(tomorrow-themes--frame-parameter-specs)
          ,@(tomorrow-themes--face-specs)))
...
;;;###autoload
(defun tomorrow-themes-night ()
  "Apply the tomorrow night theme."
  (interactive)
  (tomorrow-themes 'night))

;;;###autoload
(defun tomorrow-themes-day ()
  "Apply the tomorrow day theme."
  (interactive)
  (tomorrow-themes 'day))

;;;###autoload
(defun tomorrow-themes-bright ()
  "Apply the tomorrow bright theme."
  (interactive)
  (tomorrow-themes 'bright))

;;;###autoload
(defun tomorrow-themes-eighties ()
  "Apply the tomorrow eighties theme."
  (interactive)
  (tomorrow-themes 'eighties))

;;;###autoload
(defun tomorrow-themes-blue ()
  "Apply the tomorrow blue theme."
  (interactive)
  (tomorrow-themes 'blue))

(provide 'tomorrow-themes)

;;; tomorrow-themes.el ends here
